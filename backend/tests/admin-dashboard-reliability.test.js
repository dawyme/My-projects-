...
assert.match(layout, /await\s+auth\.logout\(\)/, 'Admin shell logout must delegate to auth.logout');
assert.doesNotMatch(layout, /auth\.clear\(\)/, 'Admin shell must not call the nonexistent auth.clear method');
assert.match(layout, /document\.querySelectorAll\('\.nav-link'\)/, 'Navigation highlighting must evaluate every nav link');
...
assert.match(dashboardPage, /data\.recurring/, 'Dashboard must consume the recurring appointment dataset');
