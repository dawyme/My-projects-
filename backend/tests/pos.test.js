...
try{
 ...
 r=await admin.post('/api/pos/sales',{customerId,items:[{productId,quantity:2}],discount:5,taxRate:12.5,paymentMethod:'CASH',location:'shop'});assert.strictEqual(r.status,201,JSON.stringify(r.body));saleId=r.body.data.id;assert.strictEqual(r.body.data.businessId,'default');assert.strictEqual(r.body.data.subtotal,250);assert.strictEqual(r.body.data.discount,5);assert.strictEqual(r.body.data.total,275.625);r=await admin.get(`/api/products/${productId}`);assert.strictEqual(r.body.data.quantity,8);
 r=await admin.post('/api/pos/refunds',{saleId,items:[{saleLineItemId:r.body.data.id,quantity:1}],reason:'Regression test'});assert.strictEqual(r.status,201,JSON.stringify(r.body));
...
}catch(e){console.error('POS regression: FAIL',e.stack||e);process.exitCode=1}finally{try{if(saleId)await prisma.sale.delete({where:{id:saleId}})}catch{}try{if(productId)await prisma.product.delete({where:{id:productId}})}catch{}try{if(customerId)await prisma.customer.delete({where:{id:customerId}})}catch{}try{if(secondBusinessId)await prisma.business.delete({where:{id:secondBusinessId}})}catch{}server.close()}}
main();
