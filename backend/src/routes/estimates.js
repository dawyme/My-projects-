...
router.get('/', protect, validate(paginationSchema.extend({ search: z.string().optional(), status: z.string().optional() }), 'query'), asyncHandler(async (req, res) => {
  ...
  const where = {};
  if (q.search) where.OR = [{ name: { contains: q.search } }, { email: { contains: q.search } }, { reference: { contains: q.search } }];
  if (q.status) where.employmentStatus = q.status.toUpperCase();
  const [items, total] = await Promise.all([
    prisma.estimate.findMany({ where, skip: (q.page-1)*q.limit, take: q.limit, orderBy: { name: 'asc' } }),
...
router.post('/', protect, adminOnly, asyncHandler(async (req, res) => {
  const t = await prisma.estimate.create({ data: req.body });
  res.status(201).json({ success: true, data: t });
}));

router.put('/:id', protect, adminOnly, asyncHandler(async (req, res) => {
  const t = await prisma.estimate.update({ where: { id: req.params.id }, data: req.body });
