...
router.get('/', protect, validate(paginationSchema.extend({ search: z.string().optional(), status: z.string().optional() }), 'query'), asyncHandler(async (req, res) => {
  ...
  const [items, total] = await Promise.all([
    prisma.invoice.findMany({ where, skip: (q.page-1)*q.limit, take: q.limit, orderBy: { name: 'asc' } }),
...
router.post('/', protect, adminOnly, asyncHandler(async (req, res) => {
  const t = await prisma.invoice.create({ data: req.body });
  res.status(201).json({ success: true, data: t });
}));

router.put('/:id', protect, adminOnly, asyncHandler(async (req, res) => {
  const t = await prisma.invoice.update({ where: { id: req.params.id }, data: req.body });
