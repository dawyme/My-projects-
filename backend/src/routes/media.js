...
router.get('/', protect, asyncHandler(async (req, res) => {
  ...
  const folders = await prisma.mediaAsset.groupBy({ by: ['folder'], _count: { _all: true } });
...
router.post('/upload', protect, upload.array('images', 8), asyncHandler(async (req, res) => {
  ...
  for (const file of req.files) {
    ...
    const asset = await prisma.mediaAsset.create({ data: { ...meta, alt: req.body.alt || null } });
    assets.push(asset);
  }
...
router.patch('/:id', protect, asyncHandler(async (req, res) => {
  ...
  const asset = await prisma.mediaAsset.findUnique({ where: { id: req.params.id } });
  if (!asset) throw notFound('Media asset not found');
  const updated = await prisma.mediaAsset.update({ where: { id: req.params.id }, data: parsed.data });
  cache.invalidate('public:content');
...
router.post('/:id/replace', protect, upload.single('image'), asyncHandler(async (req, res) => {
  ...
  const updated = await prisma.mediaAsset.update({ where: { id: req.params.id }, data: meta });
  cache.invalidate('public:content');
