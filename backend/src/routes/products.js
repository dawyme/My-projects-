...
async function uniqueSlug(req, base, ignoreId) {
  let slug = base || 'product';
  let n = 1;
  // Slug collisions get a numeric suffix (uniqueness is per tenant).
  while (true) {
    const found = await prisma.product.findFirst({ where: tenantWhere(req, { slug, NOT: { id: ignoreId || undefined } }) });
...
router.post('/', protect, validate(productBody), asyncHandler(async (req, res) => {
  const body = req.body;
  const category = await prisma.category.findFirst({ where: tenantWhere(req, { id: body.categoryId }) });
  if (!category) throw badRequest('Category does not exist');
  const sku = body.sku.toUpperCase();
  const duplicateSku = await prisma.product.findFirst({ where: tenantWhere(req, { sku }) });
  if (duplicateSku) throw badRequest('A product with that SKU already exists in this business');
  const product = await prisma.product.create({
    data: {
      ...body,
      businessId: req.tenantId,
      sku,
      slug: await uniqueSlug(req, slugify(body.name)),
...
router.put('/:id', protect, validate(productBody.partial()), asyncHandler(async (req, res) => {
  const existing = await prisma.product.findFirst({ where: tenantWhere(req, { id: req.params.id }) });
  if (!existing) throw notFound('Product not found');
  const data = { ...req.body };
  if (data.categoryId) {
    const category = await prisma.category.findFirst({ where: tenantWhere(req, { id: data.categoryId }) });
    if (!category) throw badRequest('Category does not exist');
  }
  if (data.sku) {
    data.sku = data.sku.toUpperCase();
    const clash = await prisma.product.findFirst({ where: tenantWhere(req, { sku: data.sku, NOT: { id: existing.id } }) });
    if (clash) throw badRequest('A product with that SKU already exists in this business');
  }
  if (data.name && data.name !== existing.name) data.slug = await uniqueSlug(req, slugify(data.name), existing.id);
...
router.delete('/:id', protect, adminOnly, asyncHandler(async (req, res) => {
  const product = await prisma.product.findFirst({ where: tenantWhere(req, { id: req.params.id }), include: { orderItems: true } });
  if (!product) throw notFound('Product not found');
...
router.post('/bulk-delete', protect, adminOnly,
  ...
  asyncHandler(async (req, res) => {
    const tenantIds = (await prisma.product.findMany({ where: tenantWhere(req, { id: { in: req.body.ids } }), select: { id: true } })).map((p) => p.id);
    const withHistory = await prisma.orderItem.findMany({
      where: { productId: { in: tenantIds } }, select: { productId: true }, distinct: ['productId'],
    });
    const protectedIds = withHistory.map((i) => i.productId);
    const deletable = tenantIds.filter((id) => !protectedIds.includes(id));
    const products = await prisma.product.findMany({ where: tenantWhere(req, { id: { in: deletable } }), select: { imageUrl: true } });
    const [{ count }] = await Promise.all([
      prisma.product.deleteMany({ where: tenantWhere(req, { id: { in: deletable } }) }),
      protectedIds.length
        ? prisma.product.updateMany({ where: tenantWhere(req, { id: { in: protectedIds } }), data: { isActive: false } })
        : Promise.resolve(),
    ]);
...
router.post('/bulk-update', protect, validate(bulkUpdate), asyncHandler(async (req, res) => {
  ...
  const scope = tenantWhere(req, { id: { in: ids } });
  if (direct.categoryId) {
    const category = await prisma.category.findFirst({ where: tenantWhere(req, { id: direct.categoryId }) });
    if (!category) throw badRequest('Category does not exist');
  }
  if (Object.keys(direct).length) {
    const r = await prisma.product.updateMany({ where: scope, data: direct });
    updated = r.count;
  }
  if (priceAdjustPercent !== undefined) {
    const products = await prisma.product.findMany({ where: scope, select: { id: true, price: true } });
