...
export async function render(view, query) {
    ...
    <section class="card">
      ...
      <div class="table-wrap"><table class="data"><caption class="sr-only">Orders</caption>
        <thead><tr><th scope="col">Reference</th><th scope="col">Customer</th><th scope="col">Items</th>
          <th scope="col">Placed</th><th scope="col">Status</th><th scope="col" class="num">Total</th>
  ...
  async function load() {
    ...
    try {
      ...
      if (!data.length) { rows.innerHTML = `<tr><td colspan="7">${emptyState('No orders', 'Parts orders you record will appear here.')}</td></tr>`; qs('#pager', view).innerHTML = ''; return; }
    ...
    } catch (e) { rows.innerHTML = `<tr><td colspan="7">${emptyState('Could not load orders', e.message)}</td></tr>`; }
  }
  ...
  rows.addEventListener('click', async (e) => {
    ...
    if (btn.dataset.act === 'view') {
      const m = modal({ title: 'Order details', body: '<div style="display:grid;place-items:center;min-height:180px"><div class="spinner"></div></div>', footer: '<button class="btn btn--ghost" data-close>Close</button>' });
      try {
        ...
        m.body.innerHTML = `<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px">
          ...
          <dl class="kv" style="margin-bottom:16px">
            ...
            <dt>Placed</dt><dd>${esc(dateTime(o.createdAt))}</dd></dl>
